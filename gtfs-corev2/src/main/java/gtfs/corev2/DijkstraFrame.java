...
public class DijkstraFrame extends JFrame{
	...
	public DijkstraFrame(Graph<GTFSVertex, GTFSEdge> g, Tuple<GTFSVertex, GTFSVertex> travel) {
		super("Path Visualization of Dijkstra Shortest Path");
		...
		if (travel.first != null && travel.second != null) {
		    ...
		    System.out.println("(DijkstraShortestPath) La distance totale est : "+mxTravel.second.toString()+" km");
		} else {
	...
	private Tuple<mxGraph, Double> computeMxGraph(List<GTFSVertex> path) {
	    ...
	    try {
	    	Object v1 = mxPath.insertVertex(parent, null, path.get(0).toString(), offsetX, offsetY, widthCell, heightCell, mxConstants.STYLE_FILLCOLOR + "=#ff0000");
	    	count++;
	    	...
	    	for (int i = 1; i < path.size(); i++) {
	    		...
	    		Object v2 = mxPath.insertVertex(parent, null, path.get(i).toString(), newOffsetX, newOffsetY, widthCell, heightCell, mxConstants.STYLE_FILLCOLOR + "=#ff0000");
